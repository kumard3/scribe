#!/usr/bin/env ruby
# Adds the VoxWidget WidgetKit extension target to ios/Vox.xcodeproj (run with /usr/bin/ruby).
require 'xcodeproj'
...
ext = project.new_target(:app_extension, 'VoxWidget', :ios, '16.4', nil, :swift)

group = project.main_group.find_subpath('VoxWidget', true)
group.set_source_tree('SOURCE_ROOT')
swift_ref = group.new_reference('VoxWidget/VoxWidget.swift')
group.new_reference('VoxWidget/Info.plist')
ext.source_build_phase.add_file_reference(swift_ref)

ext.build_configurations.each do |config|
  s = config.build_settings
  s['PRODUCT_BUNDLE_IDENTIFIER'] = 'ai.localvoice.app.VoxWidget'
  s['PRODUCT_NAME'] = '$(TARGET_NAME)'
  s['INFOPLIST_FILE'] = 'VoxWidget/Info.plist'
...
project.save
puts "Added VoxWidget target. Targets now: #{project.targets.map(&:name).join(', ')}"
