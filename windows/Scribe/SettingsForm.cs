...
sealed class SettingsForm : Form
{
  ...
  static readonly (string Label, string Id)[] GpuChoices =
  {
    ("Auto (use GPU if present)", GpuRuntime.Auto),
    ("NVIDIA CUDA", "cuda"),
    ("DirectML (AMD / Intel / NVIDIA)", "directml"),
    ("CPU only", "cpu"),
  };
    ...
    Func<ModelSpec, Task> switchModel, Action<Action<int>> armCapture)
  {
    ...
    layout.Controls.Add(Section("HOTKEY",
      ...
      _handsFree));
    foreach (var (label, _) in GpuChoices) _gpu.Items.Add(label);
    ...
    _gpu.SelectedIndexChanged += async (_, _) =>
    {
      if (_syncing || _gpu.SelectedIndex < 0) return;
      var id = GpuChoices[_gpu.SelectedIndex].Id;
      if (id == Settings.Instance.GpuProvider) return;
      Settings.Instance.GpuProvider = id;
      Settings.Instance.Save();
      UpdateGpuNote();
      var spec = ModelCatalog.All[_model.SelectedIndex];
      try { await _switchModel(spec); UpdateGpuNote(); }
      catch (Exception ex) { _gpuNote.Text = "GPU reload failed: " + ex.Message; }
    };
  ...
  public void Sync()
  {
    ...
    var gIdx = Array.FindIndex(GpuChoices, g => g.Id == (Settings.Instance.GpuProvider ?? GpuRuntime.Auto));
    _gpu.SelectedIndex = gIdx >= 0 ? gIdx : 0;
    UpdateGpuNote();
  ...
  {
    GpuRuntime.Probe();
    _gpuNote.Text = GpuRuntime.HasGpu
      ? $"Detected {GpuRuntime.Adapter}. Active provider: {GpuRuntime.Used}."
